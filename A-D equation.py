...
x_l   = torch.zeros_like(t_bc)   # x = 0
x_r   = torch.ones_like(t_bc)    # x = 1
...
def new_res():
    ...
    # x already in [0,1], t already in [0,1] — no remapping
...
def rar_resample(model):
    ...
    with torch.no_grad():
        # Sample in [0,1] x [0,1] — no domain shift
        ...
        # AD residual: u_t + c*u_x - nu*u_xx
        mag  = (u_t + c * u_x - nu * u_xx).abs().squeeze()
    ...
    # fill already in [0,1] — no shift needed
...
print(f"\n  Restoring best checkpoint — loss = {best_loss:.3e}")
...
def get_fdm_truth(c_val, nu_val, nx=512, nt=120):
    ...
    def rhs(u, t_val):
        ...
        u_x  = (uf[2:] - uf[:-2]) / (2*dxg)
        # AD: du/dt = -c*u_x + nu*u_xx
        return -c_val * u_x + nu_val * u_xx
...
print(f"\nBest checkpoint loss : {best_loss:.3e}")
print("\nAll done.")
